...
def getLocation(trial):
    token = int(trial['tokenId'])

    if token < 3:
        return "bottom"
    elif token > 3 and token < 7:
        return "top"
    else:
        return "hinge"
...
with open(OUT_FILE, 'w', newline='') as out:
    ...
    writer.writerow(["participantNo", 'task', 'condition', 'forCond', 'mode', 'block', 'oldBlock', 'cycleNo', 'cycleType', 'trialNos',
     'switchingTime', 'switchMissCount', 'msToken','location', 'wrongModeCount', 'posMissCount'])
        ...
        with open(OUT_FILE, 'a', newline='') as out:
            ...
            if (test[0]['taskType'] != 'baseline'):
                ...
                for i in range(0, len(test) - 1, 2):
                    ...
                    missCount = getMissCount(test[i], test[i + 1])
                    location = getLocation(test[i+1])
                    missCount, wrongCount, posCount = getMissCount(test[i], test[i + 1])

                    if int(test[i]['block']) % 2 == 1 and int(test[i]['block']) != 1:
                        block = int(test[i]['block']) // 2 + 1 
                    elif int(test[i]['block']) != 1:
                        block = int(test[i]['block']) // 2

                    if test[i]['pNo'].isdigit():
                        writer.writerow([test[i]['pNo'], test[i]['taskType'], 
                            test[i]['condition'], forCond, test[i]['currMode'], block, test[i]['block'],
                            cycleNo, 'A',
                            str(test[i]['trialNo'] + 1) + "," + str(test[i]['trialNo'] + 2), 
                            switchTime, missCount, test[i+1]['tokenId'], location, wrongCount, posCount])
                    cycleNo = cycleNo + 1
                    ...
                    if (i + 2) < len(test):
                        ...
                        missCount2 = getMissCount(test[i + 1], test[i + 2])
                        location2 = getLocation(test[i+2])
                        missCount2, wrongCount2, posCount2 = getMissCount(test[i + 1], test[i + 2])
                        #print(switchTime2)
                        if test[i]['pNo'].isdigit():
                            writer.writerow([test[i + 1]['pNo'], test[i + 1]['taskType'], 
                            test[i + 1]['condition'], forCond, test[i + 1]['currMode'], block, test[i + 1]['block'],
                            cycleNo, 'B',
                            str(test[i + 1]['trialNo'] + 1) + "," + str(test[i + 1]['trialNo'] + 2), 
                            switchTime2, missCount2, test[i+2]['tokenId'], location2, wrongCount2, posCount2])
                        cycleNo = cycleNo + 1
                ...
                for i in range(0, len(test) - 1, 2):
                    ...
                    if test[i]['pNo'].isdigit():
                        ...
                        writer.writerow([test[i]['pNo'], test[i]['taskType'], 
                            test[i]['condition'], forCond, test[i]['currMode'], test[i]['block'], test[i]['block'],
                            cycleNo, 'A',
                            str(test[i]['trialNo'] + 1) + "," + str(test[i]['trialNo'] + 2), 
                            switchTime, missCount,"none", 'none', wrongCount, posCount])